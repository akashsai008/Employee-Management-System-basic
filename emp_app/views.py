...
def dropdown_emp(request):
    if request.method == 'POST':
        emp_id = request.POST['employee']
        emp = Employee.objects.get(id=emp_id)
        emp_details = (
            f"Name: {emp.firstname} {emp.lastname}<br>"
            f"Salary: {emp.salary}<br>"
            f"Bonus: {emp.bonus}<br>"
            f"Phone Number: {emp.phonenum}<br>"
            f"Department: {emp.dept.name}<br>"  
            f"Role: {emp.role.name}"  
        )
        return HttpResponse(emp_details)
    else:
        employees = Employee.objects.all()
        return render(request, 'dropdown_emp.html', {'employees': employees})
from django.shortcuts import render
from django.http import HttpResponse
